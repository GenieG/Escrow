...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ESCO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zESCO send
enum ZerocoinSpendStatus {
    ZESCO_SPEND_OKAY = 0,                            // No error
    ZESCO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZESCO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZESCO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZESCO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZESCO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZESCO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZESCO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZESCO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZESCO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZESCO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZESCO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZESCO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZESCO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZESCO_SPENT_USED_ZESCO = 14,                      // Coin has already been spend
    ZESCO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZESCO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZESCOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzESCOWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzESCOTracker> zpivTracker;
    ...
    void setZWallet(CzESCOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzESCOTracker>(new CzESCOTracker(strWalletFile));
    }

    CzESCOWallet* getZWallet() { return zwalletMain; }
    ...
    /** zESCO reset */
    boost::signals2::signal<void()> NotifyzESCOReset;
