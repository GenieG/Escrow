...
CzESCOWallet::CzESCOWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZESCOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZESCOSeed_deprecated()) {
                    LogPrintf("%s: Updated zESCO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzESCOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZESCOCount(nCountLastUsed);
    else if (!walletdb.ReadZESCOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzESCOWallet::Lock()
{
    seedMaster = 0;
}

void CzESCOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzESCOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZESCO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzESCOWallet::LoadMintPoolFromDB()
{
...
void CzESCOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzESCOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzESCOWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzESCOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZESCO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzESCOWallet::SeedToZESCO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzESCOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzESCOWallet::GenerateDeterministicZESCO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzESCOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzESCOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
