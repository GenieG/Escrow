...
public:
    ...
    bool WriteZESCOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZESCOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZESCOSeed_deprecated(uint256& seed);
    bool EraseZESCOSeed();
    bool EraseZESCOSeed_deprecated();

    bool WriteZESCOCount(const uint32_t& nCount);
    bool ReadZESCOCount(uint32_t& nCount);
