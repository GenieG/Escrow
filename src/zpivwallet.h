...
#ifndef Escrow_ZESCOWALLET_H
#define Escrow_ZESCOWALLET_H
...
public:
    CzESCOWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZESCO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZESCO(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Escrow_ZESCOWALLET_H
