...
#if defined(NDEBUG)
#error "Escrow cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zESCO is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zESCO spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zESCO without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zESCO spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zESCO spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zESCO spend tx %s already in block %d",
            ...
            // Check that zESCO mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight < 10000 && nHeight > 201) {
        nSubsidy = 1.25 * COIN;
    }
    else if (nHeight <= 24999 && nHeight >= 10000) {
        nSubsidy = 1.5 * COIN;
    }
    else if (nHeight <= 39999 && nHeight >= 25000) {
        nSubsidy = 2 * COIN;
    }
    else if (nHeight <= 54999 && nHeight >= 40000) {
        nSubsidy = 2.5 * COIN;
    }
    else if (nHeight <= 69999 && nHeight >= 55000) {
        nSubsidy = 3 * COIN;
    }
    else if (nHeight <= 84999 && nHeight >= 70000) {
        nSubsidy = 3.5 * COIN;
    }
    else if (nHeight <= 99999 && nHeight >= 85000) {
        nSubsidy = 4 * COIN;
    }
    else if (nHeight <= 149999 && nHeight >= 100000) {
        nSubsidy = 4.5 * COIN;
    }
    else if (nHeight <= 129999 && nHeight >= 115000) {
        nSubsidy = 5 * COIN;
    }
    else if (nHeight <= 144999 && nHeight >= 130000) {
        nSubsidy = 5.5 * COIN;
    }
    else if (nHeight <= 159999 && nHeight >= 145000) {
        nSubsidy = 6 * COIN;
    }
    else if (nHeight <= 174999 && nHeight >= 160000) {
        nSubsidy = 6.5 * COIN;
    }
    else if (nHeight <= 189999 && nHeight >= 175000) {
        nSubsidy = 7 * COIN;
    }
    else if (nHeight <= 204999 && nHeight >= 190000) {
        nSubsidy = 7.5 * COIN;
    }
    else if (nHeight <= 219999 && nHeight >= 205000) {
        nSubsidy = 8 * COIN;
    }
    else if (nHeight <= 234999 && nHeight >= 220000) {
        nSubsidy = 7.5 * COIN;
    }
    else if (nHeight <= 249999 && nHeight >= 235000) {
        nSubsidy = 7 * COIN;
    }
    else if (nHeight <= 264999 && nHeight >= 250000) {
        nSubsidy = 6.5 * COIN;
    }
    else if (nHeight <= 279999 && nHeight >= 265000) {
        nSubsidy = 6 * COIN;
    }
    else if (nHeight <= 294999 && nHeight >= 280000) {
        nSubsidy = 5.5 * COIN;
    }
    else if (nHeight >= 295000) {
        nSubsidy = 5 * COIN;
    }
    else {
        nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZESCOStake)
{
    int64_t ret = 0;
    ...
    else if (nHeight <= 309999 && nHeight >= 295000) {
        ret = blockValue * 0.795;
    }
    else if (nHeight <= 324999 && nHeight >= 310000) {
        ret = blockValue * 0.80;
    }
    else if (nHeight <= 339999 && nHeight >= 325000) {
        ret = blockValue * 0.805;
    }
    else if (nHeight <= 354999 && nHeight >= 340000) {
        ret = blockValue * 0.81;
    }
    else if (nHeight <= 369999 && nHeight >= 355000) {
        ret = blockValue * 0.815;
    }
    else if (nHeight <= 384999 && nHeight >= 370000) {
        ret = blockValue * 0.82;
    }
    else if (nHeight <= 399999 && nHeight >= 385000) {
        ret = blockValue * 0.825;
    }
    else if (nHeight <= 414999 && nHeight >= 400000) {
        ret = blockValue * 0.83;
    }
    else if (nHeight <= 429999 && nHeight >= 415000) {
        ret = blockValue * 0.835;
    }
    else if (nHeight <= 444999 && nHeight >= 430000) {
        ret = blockValue * 0.84;
    }
    else if (nHeight <= 459999 && nHeight >= 445000) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Escrow
...
{
    RenameThread("escrow-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZESCOMinted()
{
...
void RecalculateZESCOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zESCO supply
...
bool RecalculateESCOSupply(int nHeightStart)
{
...
{
    // Escrow: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZESCOSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZESCOMinted();
        RecalculateZESCOSpent();
        RecalculateESCOSupply(Params().Zerocoin_StartHeight());
    }

    //Track zESCO money supply in the block index
    if (!UpdateZESCOSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zESCO supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ESCO to zESCO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zESCO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zESCO serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zESCO stake block is less than allowed start height", __func__);

    if (CZPivStake* zESCO = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zESCO->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zESCO stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zESCO stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zESCO->GetValue()));
        if (nChecksum200 != zESCO->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zESCO->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZESCO() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zESCO fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zESCO mints and %d zESCO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Escrow: We use certain sporks during IBD, so check to see if they are
